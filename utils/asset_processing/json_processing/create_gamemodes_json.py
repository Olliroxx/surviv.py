def create_game_modes(script, root_dir):
 ...
 {12}'use strict';
 {12}var _0x[\da-f]{4,6} = (\{(?:
 {16}'.*': _0x[\da-f]{4,6}\('[\da-f]{8}'\),)+
 {16}'.*': _0x[\da-f]{4,6}\('[\da-f]{8}'\)
 {12}});
 {12}_0x[\da-f]{4,6}\['exports'] = _0x[\da-f]{4,6};
 {8}},"""
    ...
    simple_replacements = {
        r'_0x[\da-f]{4,6}\["Plane"]\["Airdrop"]': "1",
        r"\\x": r"\\\\x"
    }
    for friendly_name, name in names.items():
        function_regex = " {8}'" + name + r"""': function\(_0x[\da-f]{4,6}, _0x[\da-f]{4,6}, _0x[\da-f]{4,6}\) {
 {12}'use strict';
.*? = ({.*?);"""
    ...
    for friendly_name, name in derived_function_names.items():
        dependencies_regex = " {8}'" + name + r"""': function\(_0x[\da-f]{4,6}, _0x[\da-f]{4,6}, _0x[\da-f]{4,6}\) {
 {12}'use strict';
 ...
 {16})+_0x[\da-f]{4,6} = _0x[\da-f]{4,6}\('[\da-f]{8}'\))"""
        ...
        for assignment in deps_string.split(",\n" + " "*16):
            pair = re.split(r" = _0x[\da-f]{4,6}\('", assignment)
            deps_pairs[pair[0]] = pair[1][:-2]
        merge_regex = " {8}'" + name + r"""': function\(_0x[\da-f]{4,6}, _0x[\da-f]{4,6}, _0x[\da-f]{4,6}\) {
 {12}'use strict';
.*? = {.*?};
 {12}_0x[\da-f]{4,6}\['exports'] = _0x[\da-f]{4,6}\['mergeDeep'\]\(\{}, (_0x[\da-f]{4,6}), _0x[\da-f]{4,6}"""
